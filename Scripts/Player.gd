...
func handle_jumping(var _delta):
	if(coyote_time and Input.is_action_just_pressed("shot")):
		vSpeed = jump_height
		is_jumping = true
		can_double_jump = true
	
	if(touching_ground):
		if((Input.is_action_just_pressed("shot") or air_jump_pressed) and !is_jumping):
			vSpeed = jump_height
			is_jumping = true
			touching_ground = false
		...
		if(can_double_jump and Input.is_action_just_pressed("shot") and !coyote_time):
